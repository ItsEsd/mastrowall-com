...
function ctrlqeducon(e) {
  ...
  if (res != "ID not found!") {
    ...
    document.getElementById("empower").value =
...
function logas_edu() {
  ...
  var url =
    ...
    ured2 +
    "/exec" +
    "?callback=ctrlqeduconvid&checid=" +
    logas +
    "&action=rdeduc";
  var request = jQuery.ajax({
    crossDomain: true,
...
function ctrlqeduconvid(e) {
  ...
  if (res != "ID not found!") {
    ...
    document.getElementById("m_id_lec").innerHTML =
      "Name: " + res[0].FName + " " + res[0].LName;
    document.getElementById("empowered").value =
      res[0].FName + " " + res[0].LName;
...
var urco1 = "https://script.google.com/macros/s/";
var urco2 =
  "AKfycbxdN-3O6TgqnjSFLot6a3ePPXRZvxQFWNuZ1An7Ep5_f2cyLezJ-MDd6HeW0MV7gnrC";
...
function PreviewThumb() {
  ...
  document.getElementById("preview").innerHTML =
    '<br><div id="preview_thumb" align="center"><a target="_blank" href="' +
    refaddress +
    '"><img id="preview_img" onerror="failed_img_load()" onload="success()" width="240px" height="200px" src="' +
    thumbnail +
    '"><h5 class="a" style="padding-top:5px;">' +
    title +
    "</a></h5></div>";
}

function failed_img_load() {
  document.getElementById("preview_thumb").innerHTML =
    '<div style="min-height:400px;"><img width="220px" height="210px" style="object-fit:cover;" src="https://pbs.twimg.com/profile_images/425274582581264384/X3QXBN8C.jpeg"><br><p id="error_msg" align="center" style="color:red;" >Image failed to load!</p></div>';
  ...
  $.getJSON(url, function (json) {
    ...
    for (r; r <= resultperList; r++) {
      document.getElementById("showhtml").innerHTML +=
        ...
        json.records[r].PostAs +
        " </span></div>" +
        '<input class="empid_n" value="' +
        json.records[r].EmpId +
    ...
    $(btn_notesmore).on("click", function (e) {
      if (ListIndex < numberofList) {
        ...
        for (i; i <= currentIndex; i++) {
          $("#showhtml").append(
            '<div class="iteam"><div class="card " align="center"> <div class="card-title "id="cardheader"><a target="_blank" href="' +
              json.records[i].NotesLink +
              '"><img class="swall card-img-top" id="thumbPic" onerror="this.onerror=null;this.src= `../images/thumbnail.png`;" src="' +
              json.records[i].NotesPic +
              '" height="220px" ></a></div> <div class="card-des"><a target="_blank" href="' +
              json.records[i].NotesLink +
              '"><h5 style="color:#0c09c9;">' +
              json.records[i].NotesTitle +
              '</h5></a></div> <div class="card-footer"><p style="color:#44cc00;"><span>&#9673; </span><span>' +
              json.records[i].Subject +
              ' </span><span style="color:#e65c00">&#9673; </span><span style="color:#e65c00">' +
              json.records[i].Grade +
              '</span></p><span>Posted as <svg width="20px" height="20px" style="padding-right:4px;color:#0c09c9;" viewBox="0 0 16 16" class="bi bi-clipboard-check" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/><path fill-rule="evenodd" d="M9.5 1h-3a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3zm4.354 7.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 9.793l2.646-2.647a.5.5 0 0 1 .708 0z"/></svg></span><span style="color:#0c09c9;cursor:pointer;" class="empnotenam" onclick="showpoasnote(this);">' +
              json.records[i].PostAs +
              " </span></div>" +
              '<input class="empid_n" value="' +
...
function imageFailedLoad_note() {
  document.getElementsByClassName("card-title").innerHTML =
    '<img width="220px" height="210px" style="object-fit:cover;" src="https://pbs.twimg.com/profile_images/425274582581264384/X3QXBN8C.jpeg"><br>';
}
...
function PreviewThumb_lec() {
  ...
  var myId = getId(link);
  if (link != "" && link != "http://") {
    document.getElementById("preview_lec").innerHTML =
      '<br><iframe width="100%" src="//www.youtube.com/embed/' +
      myId +
      '" frameborder="0" allowfullscreen onerror="failed_img_load_lec()" onload="success_lec()"></iframe><h5 class="a" style="padding-top:5px;" >' +
      title +
      "</a></h5>";
  } else {
    return false;
...
function failed_img_load_lec() {
  document.getElementById("preview_thumb_lec").innerHTML =
    '<div style="min-height:400px;"><img width="220px" height="210px" style="object-fit:cover;" src="https://pbs.twimg.com/profile_images/425274582581264384/X3QXBN8C.jpeg"><br><p id="error_msg" align="center" style="color:red;" >Video failed to load!</p></div>';
...
function insert_value_lec() {
  ...
  if (row1 != 0 && row2 != 0 && row4 != 0 && row5 != 0) {
    ...
    document.getElementById("Insertion_lec").innerHTML =
...
function show_wall_lec() {
  ...
  $.getJSON(url, function (json) {
    ...
    for (v; v <= resultperList; v++) {
      ...
      document.getElementById("showhtml_lec").innerHTML +=
        '<div class="iteamvid "> <div class="card " align="center"><a class="youvidframeout" onclick="showyouvid(this)";"> <div class="card-title holds-iframe">' +
        '<img class="youvubthumb" onerror="this.onerror=null;this.src= `../images/thumbnail.png`;"  src="http://img.youtube.com/vi/' +
        vidid +
        '/0.jpg">' +
        '<input class="youvidid" value="' +
        vidid +
        '" style="display:none;">' +
        '</div> <div class="card-des"> <h5 style="color:#0c09c9;">' +
        title +
        '</h5> </div></a><div class="card-footer"> <p style="color:#44cc00;"><span>&#9673; </span><span>' +
        sub +
        ' </span><span style="color:#e65c00">&#9673; </span><span style="color:#e65c00">' +
        grade +

...
function resetGame() {
  ...
  if (timerType == "countdown") {
    ...
    if (useProgressBar) {
      ...
      id("digital-timer-container").innerHTML = `<div id="digital-timer">
      ...
      ).innerHTML = `<div id="progress-bar" class="green"></div>`;
      ...
      id("animated-timer-container").innerHTML = `<div class="base-timer">
    ...
    id("stopwatch-container").innerHTML = `<div id="stopwatch">
...
function initializeGame(inputBoard) {
  ...
  solution = board_grid_to_string(
    solveSudoku(board_string_to_grid(inputBoard))
  );
...
function startGame() {
  ...
  document.getElementsByClassName("collap-2")[0].style.display = "none";
  document.getElementsByClassName("collap-3")[0].style.display = "block";
  resetGame();
...
function updateMove() {
  if (selectedTile && selectedNum) {
    ...
    if (isCorrect(selectedTile)) {
      ...
      currentBoard[Math.floor(selectedTile.id / board_size)][
        selectedTile.id % board_size
...
function show_solution() {
  ...
  for (let i = 0; i < id("board").children.length; i++) {
    ...
    if (tile.textContent != solution[tile.id]) {
      ...
      // Update the curernt status of Sudoku board
      currentBoard[Math.floor(tile.id / board_size)][tile.id % board_size] =
        solution[tile.id];
...
document.getElementById("opnaction").addEventListener("click", openaction);

function openaction() {
  document.getElementsByClassName("collap-1")[0].style.display = "none";
  document.getElementsByClassName("collap-2")[0].style.display = "block";
}
